#!/bin/bash
# Build script for Miyoo platform using Docker (musl toolchain)
...
CONTAINER_ID=`sudo docker run -v "$(pwd)/${PROJECT_DIR}":/src -itd docker.io/miyoocfw/toolchain-static-musl:latest`
